...
class BucketDBStorage(FileSystemStorage):
    ...
    def _collection_from_relpath(self, relpath):
        collection_str = os.path.dirname(relpath)
        if (not collection_str):
            raise ImproperlyConfigured(
                "No collection string can be extracted from the file name '{}'. Has 'upload_to' not been declared in the Model field?".format(relpath)
              ) 
        return self.db(collection_str)
    ...
    def _save(self, name, content):
        ...
        if (not collection_str):
            raise ImproperlyConfigured(
                "No collection string can be extracted from the file name '{}'. Has 'upload_to' not been declared in the Model field?".format(name)
              ) 
    ...
    def delete(self, name):
        #print('storage is deleting: ' + str(name))
        full_path = self.path(name)
        #print('storage is deleting: ' + str(full_path))
        os.remove(full_path)
